...
type TestParam struct {
	...
	Details any     `db:"details" param:"details" cursorField:"details"`
}
...
func Test_sqlClausebuilder_Build(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want1:   []any{1, "test", float64(123.45), float64(67.89), "simple string"},
			want2:   " WHERE 1=1 AND id=? AND name=? AND weight=? AND height=? AND details=?;",
			want3:   []any{1, "test", float64(123.45), float64(67.89), "simple string"},
			wantErr: false,
		},
		{
		...
		},
		{
			...
			want1:   []any{int64(1), int64(2), "jack", "garland", float64(1.23)},
			want2:   " WHERE 1=1 AND id IN (?, ?) AND name IN (?, ?) AND length IN (?);",
			want3:   []any{int64(1), int64(2), "jack", "garland", float64(1.23)},
			wantErr: false,
		},
		{
		...
		},
		{
			...
			want1:   []any{int64(1), "jack", "garland"},
			want2:   " WHERE 1=1 AND id=? AND name IN (?, ?);",
			want3:   []any{int64(1), "jack", "garland"},
			wantErr: false,
		},
		{
			name: "test sort by",
		...
		},
		{
			...
			want1:   []any{int64(0), "", float64(0), false, time.Time{}, time.Time{}},
			want2:   " WHERE 1=1 AND id=? AND name=? AND length=? AND active=? AND birthday=? AND created_at=?;",
			want3:   []any{int64(0), "", float64(0), false, time.Time{}, time.Time{}},
			wantErr: false,
		},
		{
