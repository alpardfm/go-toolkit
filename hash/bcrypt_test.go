...
func TestHashBcrypt(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			password:   defaultPassword,
			cost:       35,
			messageErr: "Error: error while encode bcrypt hash, crypto/bcrypt: cost 35 is outside allowed range (4,31)",
			wantErr:    true,
			mustEquals: false,
		},
		{
			...
			password:   defaultPassword,
			cost:       3,
			messageErr: "Error: error while encode bcrypt hash, crypto/bcrypt: cost 3 is outside allowed range (4,31)",
			wantErr:    true,
			mustEquals: false,
		},
	}
...
func TestCompareBcrypt(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			hashPassword: "$2b$10$8ImmbtD6jb4J6SxU2SIAjeSgJvABYwsKRhMRR4pRQHMekqTWR3Mh.",
			password:     "123456789",
			messageErr:   "Error: error while compare bcrypt hash, crypto/bcrypt: hashedPassword is not the hash of the given password",
			wantErr:      true,
			mustEquals:   false,
		},
		{
			...
			password:     defaultPassword,
			messageErr:   "Error: error while compare bcrypt hash, crypto/bcrypt: hashedSecret too short to be a bcrypted password",
			wantErr:      true,
			mustEquals:   false,
		},
		{
